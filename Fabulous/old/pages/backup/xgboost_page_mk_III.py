...
def xgboost_predict(cf, start_date, end_date, k, color):
    ...
    ticker_symbol = cf
    #all_tickers = ["AAPL", "MST", "AMZN", "WMT", "XOM", "MCK", "ABC", "UNH", "TSLA", "NFLX"]
    # Sidebar selection
    #cf = st.selectbox('Choose your capitalist fighter!!', all_tickers)

    # Explicitly define tickers outside the conditional block
    #tickers = []

    # Check if the selected ticker is "AOTA"
    #if cf == "AOTA":
        # Set tickers to all_tickers
     #   tickers = all_tickers
    #else:
        # Set tickers to the selected ticker
     #   tickers = [cf]

    # Run the analysis for each ticker in tickers
    #for ticker in tickers:
     #   
        #xgboost_predict(ticker, start_date, end_date, k, color) 
