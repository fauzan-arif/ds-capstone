...
#def xgboost_AAPL_predict(cf, start_date, end_date, k, color):
def load_and_prepare(cf, start_date, end_date):
    ...
    market_data = yf.download(cf, start=df.index.min(), end=df.index.max())
...
df = load_and_prepare("AAPL", '2020-01-01', '2024-01-01')
...
# st.text average accuracy across folds
st.text("Average Accuracy: %.2f%%" % (cv_results.mean() * 100))
...
# st.text best hyperparameters
st.text("Best Hyperparameters:", grid_search.best_params_)
...
from sklearn.metrics import classification_report
st.text("Classification Report for Test Set:")
st.text(classification_report(y_test, y_pred_threshold))
...
tomorrow_pred_class
# st.text the predicted classification for tomorrow
st.text("Predicted Tomorrow's Classification:", tomorrow_pred_class[0])

save_model(tomorrow_pred_class, "AAPL.pickle")


#st.write(f"XGBoost predictions for {cf} from {start_date} to {end_date} with {k} days into the future.")
#return xgboost_AAPL_predict
